...
toggleDCMemorySubpalette: function (el, id, field)
	{
		...
		if (item)
		{
			if (!el.value)
			{
				...
				new Request({url: window.location.href, data: 'isAjax=1&action=toggleDCMemorySubpalette&id=' + id + '&field=' + field + '&state=1'}).send();
			}
			else
			{
				el.value = '';
				...
				new Request({url: window.location.href, data: 'isAjax=1&action=toggleDCMemorySubpalette&id=' + id + '&field=' + field + '&state=0'}).send();
			}
